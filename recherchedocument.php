                ...
                switch ($_POST['type']) {
                    ...
                    case 1:
                        if(empty($_POST['date']) && empty($_POST['nom'])) {
                            ...
                            if(count($infos) > 0) {
                                echo '<p class="success">'.count($infos).' transaction(s) trouvée(s)</p>';
                                foreach ($infos as $key => $value) {
                                    $results = date('Y-m-d',strtotime($infos[$key]['date_saisie']));

                                    $a_text = '#'.$infos[$key]['id'].' '.$infos[$key]['nom'].' - '.$infos[$key]['objet'].' - '.$results;
                                    echo '<p><a href="documents/recu.php?n='.$infos[$key]['id'].'" target="_blank">'.$a_text.'</a></p>';
                                }
                            }
                            else {
                    ...
                    case 2:
                        ...
                        if(empty($_POST['date'])) {
                            ...
                            if($_POST['periode'] == 4) {
                                ...
                                $req = 'SELECT count(*) AS total FROM r_transactions WHERE date_saisie BETWEEN "'.$date_parts[0].' 00:00:00" AND "'.$date_parts[1].' 23:59:59"';
                            }
                            else {
                                //Sinon, requête LIKE
                                $req = 'SELECT count(*) AS total FROM r_transactions WHERE date_saisie LIKE "'.$date_db.'%"';
                            }

                            $title = 'Récapitulatif des transactions ';
                            $title_error = "Aucune transaction ";

                            $lien = 'documents/recap.php?date='.$date_db;
